...
document.getElementById('download-button').addEventListener('click', () => {
    ...
    if (eventDataList.length > 0) {
        ...
        eventDataList.forEach(event => {
            ...
            if (!operationHistoryByURL[url + ':' + tabid]) {
                operationHistoryByURL[url + ':' + tabid] = [];
            }
            operationHistoryByURL[url + ':' + tabid].push(event);
        });
            ...
            if (jsonData.operationHistoryByURL && typeof jsonData.operationHistoryByURL === "object") {
                ...
                if (urls.length > 0) {
                    ...
                    if (firstOperationHistory && Array.isArray(firstOperationHistory)) {
                        ...
                        console.log(urls2[0]);
                        openAndProcessTab(urls2[0], xpaths);
                            ...
                            if (message.type === 'end-replayJson') {
                                ...
                                if (urls.length > 1) {
                                    ...
                                    if (urls.length > urlsNum) {
                                        addTabUpdateListener(message.detail.tabId, jsonData.operationHistoryByURL[urls[urlsNum]].map(item => item.currentURL)[0], jsonData.operationHistoryByURL[urls[urlsNum]].map(item => item.fullXPath), true);
                                    }
                                }
